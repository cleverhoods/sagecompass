...
from app.utils.paths import APP_DIR, CONFIG_DIR
...
class FileLoader:
    ...
    def load_yaml(cls, relative_path: str, category: str = "config"):
        """
        Load a YAML file relative to APP_DIR (app/).
    ...
    def load_prompt(cls, agent_name: str, prompt_name: str):
        ...
        file_path = os.path.join(
            APP_DIR, "agents", agent_name, f"{prompt_name}.prompt"
        )
    ...
    def load_schema(cls, agent_name: str, schema_name: str):
        file_path = os.path.join(
            APP_DIR, "agents", agent_name, f"{schema_name}.json"
        )
    ...
    def load_agent_config(cls, agent_name: str):
        ...
        """
        path = os.path.join(APP_DIR, "agents", agent_name, "config.yaml")
